# Testing — mnemo smoke tests (current: v1.2.17)
...
- **Historical feature matrix remains targeted:** the live 7×2 pass does not imply that every A- and V-check in the feature sections below had its toggles and negative branches re-run — neither the ones that predate that pass nor the ones added by later releases (the numbering keeps growing, so read the sections rather than a range). In particular, the opt-in V3 `autoStampReviewed` live scenario remains a separate targeted check; its automated writer contracts are part of the required gate.
...
Model routing was rewritten to prevent mid-session model switches from triggering `API Error: Extra usage is required for 1M context` on Max plans. Current routing: one skill (`health`) runs in an isolated forked subagent (`context: fork` + `haiku`); four (`ask`, `save`, `session`, `review`) inherit the session model (`model: inherit`); the remaining two pin a model without forking (`connect` → `sonnet`, `setup` → `haiku`). The v0.7.3 split itself is in the [CHANGELOG](./CHANGELOG.md#073---2026-04-24).
...
- **V9 — SessionStart nudge (`hooks/mnemo-context.sh`).** With a configured vault, a new session's context includes the one-line mnemo nudge. Direct smoke (from the repo root, so the path is the real one — the bare `hooks/…` form only exists plugin-relative): `bash plugins/mnemo/hooks/mnemo-context.sh` → prints `hookSpecificOutput.additionalContext`; `HOME=/tmp/empty bash …` → `{"continue":true,"suppressOutput":true}` (silent unconfigured); `hooks.sessionStartNudge:false` → silent. **(smoke-passed 2026-07-05)**
- **V10 — Stop nudge governor (`hooks/mnemo-stop-nudge.sh`, opt-in).** Default (`hooks.stopNudge` absent/false) → always `pass`, never blocks. With `hooks.stopNudge:true` + stdin `{"session_id":"x","transcript_path":"<file>"}` and ≥3 fix/decision signals: `mn:save` **or** `mn:session` missing from the transcript → `block` ONCE with a single message recommending the one-command close-out `/mn:review --full` (`$mnemo:review --full` in Codex) — since v1.2.9 the nudge no longer lists save and session separately, and the message is the same whichever of the two is missing; **both** present → `pass`; second call same session_id → `pass` (anti-loop); <3 signals → `pass`. Covered by `scripts/test-stop-nudge.py`. **(smoke-passed 2026-07-05 — save+session tracking; earlier caught + fixed a `grep -c` double-zero bug; that live pass predates the v1.2.9 message rewrite)**
...
## What changed in v1.2.16 — why the autocompact nudge is quiet

- **V25 — `context-window.py --explain` (read-only diagnostic).** `python3 plugins/mnemo/scripts/context-window.py --explain` prints one JSON object and exits 0: `resolved`, `source`, `window`, `compacts_at`, `reserve`, `clamped`, `hint`. `source` names where the window came from — `env` (`CLAUDE_CODE_AUTO_COMPACT_WINDOW`), `settings` (`autoCompactWindow` in the settings chain), `cache` (Claude Code's per-model `~/.claude.json` entry), `model-default`, or `disabled`. Three branches are worth checking by hand: `autoCompactEnabled: false` → `resolved:false` with `source:"disabled"`; nothing resolvable → `resolved:false` and a hint carrying the formula (window = desired threshold + 33000, e.g. 493000 to compact at 460000); a configured value above the model's context window → `clamped:true`, with only the ceiling in force. It must never write a settings file. Covered by the `ExplainTest` class in `scripts/test-context-window.py` (which the gate's `scripts/test-*.py` glob picks up on its own) — see V21 for that suite's count.
- **V26 — health Step 7.55 + the setup hint (live only).** With `hooks.autocompactNudge: true` in `~/.mnemo/config.json`, `/mn:health` runs Step 7.55, quotes the `hint` verbatim, and calls out `resolved:false` ("enabled but can never fire") and `clamped:true` ("above the model's window, so it does nothing") explicitly. With the nudge off — the default — the step is skipped and no autocompact line appears in the report. In `/mn:setup`, turning the nudge on must pass the same hint on to the user. Both are skill behavior rather than script behavior, so no automated suite covers them; mnemo must still ship no default for `autoCompactWindow` and never write it.

## What changed in v1.2.17 — private-leak guard

- **V24 — a link into a private vault cannot ship.** `python3 scripts/test-lint-wikilinks.py` → 22 OK. The guard lives in `scripts/lint-skills.py` and runs with every lint. It passes only shapes that cannot name a real note — placeholders (`[[{hub_name}]]`, `[[Session — …]]`), single ASCII words (`[[wikilinks]]`), regex fragments — plus the names in `scripts/wikilink-allowlist.txt`; everything else fails the lint with both ways out named in the message. Two checks are worth knowing about: the sweep reads **whole files**, because the second link caught by hand in the v1.2.14 review was wrapped across two source lines and a line-by-line sweep called that file clean; and a sweep that matches **nothing at all** fails instead of reporting clean, since a wrong root or extension list looks identical to a clean repo. The same pass flags absolute home paths — a macOS, Linux, or Windows user directory with a real account name in it — while leaving `~/` and `${VAR}` forms alone.
- **Live check:** drop a wikilink naming a concrete note (`Atom — …` with a real title in double brackets) into any `.md` in the repo and run `python3 scripts/lint-skills.py` — it must exit 1 and name that file and line. Note that this document cannot spell those examples out literally: the guard scans it too, and it caught this very section while it was being written. `test_a_planted_violation_actually_fails_the_lint` does exactly this, because asserting that the guard *prints its line* is not enough: it prints the same `✅` when the call has been deleted.
...
- **Steps 1-4 execute in parallel** (you'll see 5 `safe-read.py` calls — orphans, unresolved, top-unresolved, tags, files-total — batched into a single assistant message; Step 4 reads the tag counts from Step 3's output instead of asking for them again).
