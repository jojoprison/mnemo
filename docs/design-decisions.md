...
- **Autocompact nudge** (`hooks/mnemo-autocompact-nudge.sh`, `hooks.autocompactNudge` default **false**) — warn before autocompact summarizes the session and drops raw context. Claude Code's effective threshold is `min(configured window, the model's context window)`, so a configured value is a *ceiling request*, not the threshold: a 460k setting on a 200k session never takes effect at all. The window is resolved the way Claude Code resolves it — `CLAUDE_CODE_AUTO_COMPACT_WINDOW`, then `settings.autoCompactWindow` (project-local → project → user, `.local.json` first at both levels), then its own per-model `~/.claude.json` cache read **by the active model's key** — validated to the same `[100k, 1M]` range Claude Code accepts, then clamped to the context window. **The one place mnemo deliberately refuses to imitate Claude Code is the final fallback.** Claude Code ends its chain at the model's context window; mnemo cannot, because the transcript records the model id *without* its `[1m]` suffix, so a 1M session and a 200k session are written identically — and assuming 200k for a 1M session would block a turn at 15% full. So the ceiling is claimed only in the direction that is provable: when Claude Code's own 1M-access cache shows the account has no large-context access, the standard 200k window applies and the nudge works with no configuration at all; when access exists, the cache is missing, or its shape is unfamiliar, the hook stays silent until a window is configured explicitly. A last guard covers a wrong ceiling from any source: usage above the resolved window is impossible, so it reports "unknown" instead of a false alarm. Bands are measured from the point compaction actually happens — `W − min(max_output, 20k) − 13k` — because measured from `W` the `critical` band would sit past a threshold the session can never reach, i.e. be dead code. Token usage comes from the transcript in a single pass (a long session's file runs to hundreds of megabytes), not from hook stdin, which carries no token count — the same Stop-hook constraint as `stopNudge`'s signal scan; the sum is Claude Code's own, `input + cache_creation + cache_read`. Anti-loop: it blocks at most once per severity level, **and lowers the marker when the level drops**, since a compaction changes neither `session_id` nor transcript path — without that, the first nudge would also be the last one that session. It shares `stopNudge`'s "nothing left to rescue" gate (silent once save *and* session have run), so the two Stop hooks cannot both nag for a close-out that already happened, and stays silent when `autoCompactEnabled` is false, where its own wording would be untrue. Claude Code only: Codex has no comparable window, so the hook no-ops there. **Opt-in, default false** — same loop-risk reasoning as `stopNudge`.
