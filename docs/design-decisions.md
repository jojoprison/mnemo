...
- **Stop nudge** (`hooks/mnemo-stop-nudge.sh`, `hooks.stopNudge` default **false**) — if a session looks worth-saving (fix/decision signals) but `/mn:save` never ran, it blocks the stop **once** to prompt a save. Blocking is powerful but can loop for arbitrary users of a public plugin, so it's **opt-in**, and an anti-loop governor blocks at most once per session. The default install never blocks.
- **Codex** — `hooks/codex-hooks.json` mirrors both (Codex does consume plugin hooks; it may ignore `additionalContext` / `decision:block`, degrading to a no-op safely).
