...
- **Codex** — consumes the same `hooks/hooks.json`; Codex supplies `PLUGIN_ROOT` plus the `CLAUDE_PLUGIN_ROOT` compatibility variable. Hook scripts render `$mnemo:*` syntax under Codex. There is no forked hook manifest.
