...
Obsidian file-sync does not resolve merge conflicts when a human and an agent edit the same note (the "safe without merge-conflict resolution" claim was refuted 0-3). Never blind-overwrite a note you did not just read — use `vault-write.py replace` or `insert` against an exact verbatim anchor (or guarded append with an expected tail/hash), and re-read before editing a note another party may have touched. The optimistic write must fail rather than clobber a concurrent edit.
