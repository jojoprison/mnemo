...
| `handoff_note` | Cross-session continuity file name | session, setup, health |
...
| `hooks.autocompactNudge` | Claude Code only (no-op on Codex): at session end, if the transcript's token usage is within a fixed margin of the point autocompact actually fires (warn ~50k, critical ~10k remaining, all clamped down on a small window), block once per severity level and recommend `/mn:review --full` before autocompact drops raw context. The window follows Claude Code's own chain — env `CLAUDE_CODE_AUTO_COMPACT_WINDOW`, `settings.autoCompactWindow`, then its per-model `~/.claude.json` cache read by the active model's key — validated to `[100k, 1M]` and clamped to the model's context window, since a configured value is a ceiling request rather than the threshold. With no configured value the standard 200k window applies **only** when Claude Code's 1M-access cache proves the account has no large-context access; otherwise the hook stays silent rather than guess low. Silent as well when `autoCompactEnabled` is false, when save and session have both already run, and when usage exceeds the resolved window (which can only mean the window is wrong). The marker lowers when the level drops, so a second compaction in the same session still warns. **Opt-in, default false**, same posture as `stopNudge` | hooks/mnemo-autocompact-nudge.sh |
...
| `review.lint.model` | Model for the lint pass, spawned as a subagent so the cheap haiku health fork stays cheap. `haiku` (default, triage-grade) / `sonnet` / `opus` (highest-quality verdicts & contradiction detection — the value is a tier alias, so mnemo pins no model version). Only the lint subagent uses it; Steps 1-7 always run on the health fork's own model | health |
...
**Turning on `hooks.autocompactNudge` may need one Claude Code setting, and mnemo will not set it for you.** The nudge needs the autocompact window; when it cannot be established it stays silent, and that silence looks exactly like "nothing to warn about". `python3 "<mnemo-root>/scripts/context-window.py" --explain` reports which case you are in and what to set. Two rules decide the value:

- **The threshold is not the window.** Claude Code compacts at `window − min(max_output, 20000) − 13000` — 33000 below the window on a model whose max output is ≥ 20k. So `autoCompactWindow` = *desired threshold + reserve*: to compact at 460000, set **493000**; setting 460000 compacts at 427000. mnemo additionally clamps the reserve it assumes to a quarter of the window (`min(33000, window // 4)`), so under a 132000 window the nudge — and the `+N` in `--explain` — uses `window // 4` instead of 33000 (at the 100000 validation floor: 25000).
