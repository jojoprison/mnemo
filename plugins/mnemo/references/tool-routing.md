...
Use a static action name plus a **single-quoted herestring**:

```bash
python3 "<mnemo-root>/scripts/safe-read.py" backlinks <<< '{"file":"{vault-derived note name}","vault":"{configured vault}"}'
```
