---
name: session-review
description: "End-of-session orchestrator. Audits the session, then recommends core skills (mnemo:memory-routing, mnemo:session-notes) and the rest — always asks before running anything, never auto-runs. Triggers on 'что забыли', 'session review', 'что осталось', 'all done?', end of significant work, or similar. The ONLY command users need at session end — one confirmation covers everything."
